...
Another nice feature of `OnDemand` systems is that we can pass information to the system that is stored on stack. This keeps all data localized, which makes it likely that the code runs efficiently on multiple threads. The example uses this to pass data between systems, as is shown in the code example below.
